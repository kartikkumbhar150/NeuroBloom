...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // DEMO BYPASS
    if (email === "demo@neurobloom.com" && password === "demo") {
      setTimeout(() => {
    ...
    try {
      ...
      // For demo purposes, if backend fails, let them in if they used demo creds
      if (email === "demo@neurobloom.com") {
        router.push("/dashboard");
        return;
      }
      alert("Login failed. Try using demo@neurobloom.com / demo");
      setLoading(false);
    }
  ...
  const handleDemoLogin = () => {
    setEmail("demo@neurobloom.com");
    setPassword("demo");
    // Auto submit handled by user clicking sign in, or we can trigger it
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-indigo-100/50 p-8 md:p-10 border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 font-medium text-lg disabled:opacity-70"
            >
            ...
            <button
              ...
              className="w-full py-3.5 bg-teal-50 text-teal-700 rounded-xl hover:bg-teal-100 transition-all border border-teal-200 font-medium"
            >
              Use Demo Account (Bypass)
            </button>
          </form>
